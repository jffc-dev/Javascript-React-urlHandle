...
import AleatorioUrlsBdPageHead from '../../AleatorioUrlsBdPage/AleatorioUrlsBdPageHead/AleatorioUrlsBdPageHead.js'
import AleatorioUrlsBdPageBody from '../../AleatorioUrlsBdPage/AleatorioUrlsBdPageBody/AleatorioUrlsBdPageBody.js'
import { useParams } from 'react-router-dom'
import { getPlaylistById } from '../../../services/playlists/getPlaylist.js'
...
const DetailPlaylistPage = () => {
  ...
  const handleGetPlayList = async (playlistId) => {
    ...
    const { data, status, message } = await getPlaylistById(playlistId)
    if (status === OK_STATUS) {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      const playlist = await handleGetPlayList(playlistId)
      console.log(playlist)
      setCadenas(playlist.urls)
      setLinkCount(playlist.urls.length)
      setCurrentLink(0)
    }
    fetchData()
  }, [])
    ...
    <Container style={{ paddingTop: '80px' }}>
      ...
      <div>
        <AleatorioUrlsBdPageHead
          ...
          randomPage={false}></AleatorioUrlsBdPageHead>
        <AleatorioUrlsBdPageBody
          ...
          randomPage={false}></AleatorioUrlsBdPageBody>
      </div>
    </Container>
  )
}

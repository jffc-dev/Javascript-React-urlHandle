...
const AleatorioUrlsManualPage = () => {
  ...
  const handleContador = (operacion) => {
    if (operacion === '+') setnroResultados((prevCounter) => prevCounter + 1)
    else setnroResultados((prevCounter) => (prevCounter > 1 ? prevCounter - 1 : prevCounter))
  }
  ...
  const obtenerCadenas = () => {
    ...
    while (cadenasResultado.length < nroResultados) {
      const item = arrayCadenas[Math.floor(Math.random() * arrayCadenas.length)]
      ...
      <div className="random__contenedor">
        <div className="random__contenedor__textarea">
          <textarea
            ...
            value={cadenas}
            onChange={handleChangeCadenas}></textarea>
        </div>

...
export const ModalResource = ({opened, close}: ModalResourceProps) => {
    const { selectedResourceId } = useRandomStore(useShallow((state) => ({
        selectedResourceId: state.selectedResourceId,
    })));

    const {data: resource, loading} = useGetResource({ id: selectedResourceId });

    const {control, handleSubmit, reset, setError, formState: {isDirty, isValid, errors}} = useForm({
        mode: 'all',
